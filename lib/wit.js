...
const validateActions = (actions) => {
  ...
  Object.keys(actions).forEach(key => {
    ...
    } else if (key === 'merge' && actions.merge.length !== 5) { // DEPRECATED
    ...
    } else if (key !== 'say' && key !== 'merge' && key !== 'error' && actions[key].length !== 4) {
      l.warn('The \'' + key + '\' action should accept 4 arguments: sessionId, context, entities, callback. ' + learnMore);
    }
  });
  return actions;
};
...
const Wit = function(token, actions, logger) {
  ...
  const makeCallback = (i, sessionId, message, context, cb) => {
    ...
    return (error, json) => {
      ...
      if (json.type === 'stop') {
      ...
      } else if (json.type === 'merge') { // DEPRECATED
        ...
        this.actions[action](sessionId, clonedContext, json.entities, makeActionCallback());
      } else { // error
